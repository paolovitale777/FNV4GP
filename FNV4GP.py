...
def mean_squared_percentage_error(y_true, y_pred):
    """Mean Squared Percentage Error: the squared-error analogue of MAPE.
    No sklearn builtin exists for this, so it's implemented here directly,
    using the same zero-division guard sklearn's MAPE uses internally."""
    y_true = np.asarray(y_true, dtype=float)
    y_pred = np.asarray(y_pred, dtype=float)
    epsilon = np.finfo(np.float64).eps
    return float(np.mean(((y_true - y_pred) / np.maximum(np.abs(y_true), epsilon)) ** 2))


# FNV4GP logo icon (marker-grid -> prediction curve), embedded as base64 PNG
# so the app stays a single self-contained file.
_FNV4GP_LOGO_PNG_B64 = (
    "iVBORw0KGgoAAAANSUhEUgAAAIAAAACACAYAAADDPmHLAAAAOnRFWHRTb2Z0d2FyZQBNYXRwbG90bGliIHZlcnNpb24zLjEwLjgs"
    "IGh0dHBzOi8vbWF0cGxvdGxpYi5vcmcvwVt1zgAAAAlwSFlzAAAPYQAAD2EBqD+naQAACR1JREFUeJztnVtsFNcZx3+zF3t9pdik"
    "trIJYGMQoFRCVGCXppWoktKkTR8iEaJWshTUm5pKVZW0kDalEQ+IKOQxlVopVFopUhIkHtIUWppE6i2wTUOjXgIBY1NgJRNfIDYL"
    "u97L9GHH9s54Zi/e2Vmvz/eTVquZnXPmyP//nPOdc771arquI6iLr9YNEGqLGEBxAm5WFu4f1IAeYAPQB6w33vuATiBkvPxu3ncZ"
    "kQESxmsCGDJeF433C8BILBpxbdzWKo0BDNG3Ao8Bu8kZQKgeI8Ax4HXgbKVmWLQBwv2DHcDTwOOI6LViBHgVOBKLRiYXU0HZBgj3"
    "D/qAvcBhct26UHsmgP3A0Vg0ki2nYFkGCPcPfhb4JbC9rOYJXhEFnoxFI++XWqBkA4T7Bx8FXsPlwFFwnTSwJxaNHC/l4pKmgSJ+"
    "XREAXjM0K0pRA4j4dUnJJig4BBhj/hlE/JoQCui0hXSC/pxGqYzGdEIjkdZKrSINDBSKCRwNYET7p5GAz1NCAZ17V2a4e0WWxqC9"
    "NsmURuwTH9du+EsxQxTY4TQ7KPRk70XE9wyfprNuVYa1nRm0Ipo2BnV6V2Xo6cxwecLPpXE/Wd2xUD/wBPCy3Ye2PYCxyHMBmed7"
    "QlNQZ+u9KVoaF7coF09qnL0a5E7K0QQTwAa7xSKnIPBpRHxPaArqbF+zePEBWhpzdTQ5DBnktHzK7oMFPYCxtn8JWd6tOj5N53M9"
    "lYmfTzypcXok6DQcDAN91r0Dux5gKyK+J6xblSkivk53e4bN3Wk2d6fpbs8Azte3NObiCAd6yWlrwi4IfKxAiwSXCAV01nY6isXG"
    "rjTb1qToajMLfn1K470rQc5ft4/f13ZmuOo8O9gNmKaEpiFAun/v6LsrTa/D03r/uhkG1qYLlj8zEuCvww22nw2P+xkaszXIgmHA"
    "OgT0IOJ7QniF/abdxq50UfEBBnrSbOyyv86pbnLDgElfqwE2FL2zUDGhgO6wyKOzbU2q5Hq2rU5hFxM0BnUaA46xwvr8A6sB+kq+"
    "sml1RfV+cO6K7flqZTHXC9IDKI4YQHGsBsjYXiUsJ0waWw2QQFjumH7DXgygHiaNrQaY8LAhQm2YzD+wGmDIw4YIteFi/oEYQD1M"
    "GlsNcBFhuSM9gOIU7AEueNgQoTYU7AFGjJewPBnGoq/JALFoRAeOedkiwVOOGRrPYbcX8LpHjRG8Z8HDbWeAs8gwsBwZJqetCVNW"
    "8Czh/sFDwDMeNErwjkOxaORn1pNO28FHkGXh5cQE8KLdB7YGiEUjk8D+arZI8JR9hqYLKJQQchT4e3XaI3hIFHD8b5aOBohFI1ng"
    "+0C6Co0SvCENPGloaUvBlLBYNPI+sAcxQT2SBvYYGjpSNCcwFo0cR0xQb8yKf7zYhSUlhYoJ6oqSxYcysoKNCgfIBRXC0iQKDJQq"
    "PpSZFm6MJzuAbyHrBEuJcXKa7Cg25luxXQkshXD/YAfwFPA40LuoSoRKGQZeBV50mucXY9EGmCXcP6gBW4HdxkvMUF2GyW3qHAPO"
    "Wnf3yqViA+RjmKEHWA/0Wd47gBDQBPhdu+nyIkMubz9BLnv3IrkMnvz3kUpFz8dVAwj1h3w3UHHEAIrzf2us5nnV/sr9AAAAAElF"
    "TkSuQmCC"
)
...
class MarkerFilterApp(tk.Tk):
    def __init__(self):
        ...
        # Logo: decode once, keep references alive (Tkinter drops PhotoImages
        # with no live reference), use as window/taskbar icon.
        self._logo_full = tk.PhotoImage(data=_FNV4GP_LOGO_PNG_B64)
        self._logo_small = self._logo_full.subsample(2, 2)
        try:
            self.iconphoto(True, self._logo_full)
        except Exception:
            pass
    ...
    def _build_home_page(self, parent):
        ...
        lockup = tk.Frame(header, bg=NAV)
        lockup.pack(pady=(22, 22))

        tk.Label(lockup, image=self._logo_small, bg=NAV).pack(side="left", padx=(0, 14))

        wordblock = tk.Frame(lockup, bg=NAV)
        wordblock.pack(side="left")

        tk.Label(
            wordblock, text="FNV4GP",
...
class SummaryPAPlotPage(ttk.Frame):
    ...
    def _draw_barplot(self, stats_df):
        ...
        ax.set_title("")
        ax.set_ylabel("Predictability", fontsize=24)
        ax.set_xlabel("Model", fontsize=24)
        ax.set_xticks(x)
        ax.set_xticklabels(stats_df["Model"].astype(str).values, rotation=35, ha="right", fontsize=20)
...
class IndependentValidationPAPlotPage(ttk.Frame):
    ...
    def _draw_barplot(self, stats_df):
        ...
        ax.bar(x, y, yerr=yerr, capsize=6)
        ax.set_ylabel("Predictability (PA_test)", fontsize=24)
        ax.set_xlabel("Model", fontsize=24)
        ax.set_xticks(x)
        ax.set_xticklabels(stats_df["Model"].astype(str).values, rotation=35, ha="right", fontsize=20)
        ax.tick_params(axis='y', labelsize=20)
...
class NestedSvrCvFrame(ttk.Frame):
    ...
    def __init__(self, parent):
        ...
        # Tuning criterion
        self.tune_by = tk.StringVar(value="MAE")
    ...
    def _build_cv_ui(self):
        ...
        ttk.Combobox(cv, textvariable=self.tune_by, state="readonly",
                     values=["MAE", "MSE", "PA"], width=10).grid(row=1, column=5, sticky="w", **pad)
                ...
                for outer_fold, (tr_idx, te_idx) in enumerate(outer_cv.split(X), start=1):
                    ...
                    if args["use_gamma"]:
                        for C, epsilon, kernel, gamma in grid:
                            fold_pas, fold_mses, fold_maes, fold_true_mapes, fold_true_mspes = [], [], [], [], []
                            for itr_idx, iva_idx in inner_cv.split(X_train_outer_s):
                                ...
                                true_mspe = mean_squared_percentage_error(yva, pred)
                                mae = mean_absolute_error(yva, pred)
                                true_mape = mean_absolute_percentage_error(yva, pred)

                                fold_pas.append(pa)
                                fold_mses.append(mse)
                                fold_true_mspes.append(true_mspe)
                                fold_maes.append(mae)
                                fold_true_mapes.append(true_mape)

                            tuning_rows.append({
                                "C": C, "epsilon": epsilon, "kernel": kernel, "gamma": gamma,
                                "PA": float(np.nanmean(fold_pas)),
                                "MSE": float(np.mean(fold_mses)),
                                "MSPE": float(np.mean(fold_true_mspes)),
                                "MAE": float(np.mean(fold_maes)),
                                "MAPE": float(np.mean(fold_true_mapes))
                            })
                    else:
                    ...
                    if tune_by == "MAE":
                        ...
                        best = tuning_df.loc[tuning_df["MSE"].idxmin()]
                    else:  # PA
                    ...
                    mse = mean_squared_error(y_test_outer, y_test_pred)
                    true_mspe = mean_squared_percentage_error(y_test_outer, y_test_pred)
                    mae = mean_absolute_error(y_test_outer, y_test_pred)
                    true_mape = mean_absolute_percentage_error(y_test_outer, y_test_pred)
...
class IndependentValidationSvrFrame(ttk.Frame):
    ...
    def _build_ui(self):
        ...
        ttk.Combobox(cv, textvariable=self.tune_by, state="readonly",
                     values=["MAE", "MSE", "PA"], width=10).grid(row=0, column=5, sticky="w", **pad)
    ...
    def _worker(self, **args):
        try:
            ...
            if use_gamma:
                for C, epsilon, kernel, gamma in grid:
                    ...
                    for inner_train_idx, inner_val_idx in inner_cv.split(X_training):
                        ...
                        true_mspe = mean_squared_percentage_error(y_inner_val, y_val_pred)
                        mae = mean_absolute_error(y_inner_val, y_val_pred)
                        true_mape = mean_absolute_percentage_error(y_inner_val, y_val_pred)
            ...
            true_mspe = mean_squared_percentage_error(y_test, y_test_pred)
            mae = mean_absolute_error(y_test, y_test_pred)
            true_mape = mean_absolute_percentage_error(y_test, y_test_pred)
            ...
            result_row = {
                ...
                "Best_epsilon": float(best["epsilon"]),
                "Best_kernel": str(best["kernel"]),
                "Best_gamma": best["gamma"] if use_gamma else None,
            }
    ...
    def _worker(self, **args):
        try:
            ...
            for combo in grid:
                ...
                row["MSE"] = float(np.mean(fold_mses))
                row["MSPE"] = float(np.mean(fold_true_mspes))
                row["MAE"] = float(np.mean(fold_maes))
                row["MAPE"] = float(np.mean(fold_true_mapes))
            ...
            if tune_by == "MAE":
                ...
                best_row = tuning_df.loc[tuning_df["MSE"].idxmin()]
            else:
                best_row = tuning_df.loc[tuning_df["PA"].idxmax()]

            metric_cols = {"PA", "MSE", "MAE", "MAPE", "MSPE"}
...
class NestedRidgeCvWindow(tk.Frame):
    def __init__(self, parent, app):
        ...
        self.tune_by     = tk.StringVar(value="MAE")  # MAE / MSE / PA
